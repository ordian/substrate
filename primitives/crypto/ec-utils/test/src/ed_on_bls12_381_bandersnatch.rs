...
#[test]
fn test_ed_on_bls12_381_bandersnatch_mul_projective_runtime() {
	use ark_ed_on_bls12_381_bandersnatch::G1Projective;
...
#[test]
fn test_ed_on_bls12_381_bandersnatch_mul_affine_runtime() {
	// Get runtime client for testing
	let test_client = get_test_client().expect("Test client builds");
	...
	let result = test_client
		.runtime_api()
		.ed_on_bls12_381_bandersnatch_mul_affine(
			test_client.chain_info().genesis_hash,
			base.encode(),
			scalar.encode(),
		)
...
#[test]
fn test_ed_on_bls12_381_bandersnatch_msm_runtime() {
	use ark_ed_on_bls12_381_bandersnatch::G2Projective;
	// Get runtime client for testing
	let test_client = get_test_client().expect("Test client builds");
